...
def dedupe_rows(rows, fieldnames):
...
def merge_gw(gw, gw_directory):
    ...
    with open(gw_path, "r", encoding="utf-8") as fin:
        ...
        new_fieldnames = source_fieldnames + ["GW"]
        new_rows = []
        for row in reader:
            row["GW"] = gw
            new_rows.append(row)
    ...
    if os.path.exists(out_path) and gw != 1:
        ...
        # Replace this GW instead of appending it again.
        # This keeps merge_gw idempotent when rerun for the same gameweek.
        before = len(existing_rows)
        existing_rows = [row for row in existing_rows if str(row.get("GW", "")) != str(gw)]
        removed = before - len(existing_rows)
        if removed:
            print(f"  Removed {removed} existing GW{gw} rows before merge")
    ...
    all_rows = dedupe_rows(all_rows, final_fieldnames)

    # Write all data with unified schema
    with open(out_path, "w", encoding="utf-8", newline="") as fout:
        writer = csv.DictWriter(
            fout, fieldnames=final_fieldnames, lineterminator="\n", extrasaction="ignore"
        )
        writer.writeheader()
        for row in all_rows:
...
def regenerate_merged_gw(gw_directory):
    ...
    all_rows = dedupe_rows(all_rows, final_columns)

    # Write the merged file
    with open(out_path, "w", encoding="utf-8", newline="") as fout:
        writer = csv.DictWriter(
            fout, fieldnames=final_columns, lineterminator="\n", extrasaction="ignore"
        )
        writer.writeheader()
        for row in all_rows:
